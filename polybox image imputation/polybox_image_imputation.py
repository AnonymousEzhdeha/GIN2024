...
def generate_poly_filter_dataset( num_seqs_train, num_seqs_test, num_seqs_valid,
    ...
    cannon.run(delay=None, iterations=seq_length_train+1, sequences=num_seqs_train, radius=3*scale, angle_limits=(0, 360), shape=2,
               velocity_limits=(10.0*scale, 15.0*scale), filepath='./data/polygon.npz', save='npz')

    np.random.seed(5678)
    cannon = BallBox(dt=0.2, res=(32*scale, 32*scale), init_pos=(16*scale, 16*scale), init_std=2.5, wall=None)
    cannon.run(delay=None, iterations=seq_length_test+1, sequences=num_seqs_test, radius=3*scale, angle_limits=(0, 360), shape=2,
               velocity_limits=(10.0*scale, 15.0*scale), filepath='./data/polygon_test.npz', save='npz')
    
    np.random.seed(7788)
    cannon = BallBox(dt=0.2, res=(32*scale, 32*scale), init_pos=(16*scale, 16*scale), init_std=2.5, wall=None)
    cannon.run(delay=None, iterations=seq_length_valid+1, sequences=num_seqs_valid, radius=3*scale, angle_limits=(0, 360), shape=2,
...
class PolyImageImputGIN(GIN):
    ...
    def build_decoder_hidden(self):
        return [
            ...
            k.layers.Lambda(lambda x: tf.reshape(x, [-1, 4, 4, 16])),

            k.layers.Conv2DTranspose(16, kernel_size=5, strides=4, padding="same"),
            LayerNormalizer(),
            k.layers.Activation(k.activations.relu),

            k.layers.Conv2DTranspose(16, kernel_size=3, strides=2, padding="same"),
            LayerNormalizer(),
            k.layers.Activation(k.activations.relu)
        ]
...
def main():
    ...
    for key in configs.keys():
        ...
        train_data, test_data, valid_data, train_valid, test_valid, valid_valid = generate_poly_filter_dataset(1000, 100, 100, 20, 20, 20)

        #Build Model
        gin = PolyImageImputGIN(observation_shape=train_data.images.shape[-3:], 
                                latent_observation_dim = configs[key]["lod"],
                                latent_state_dim = configs[key]["lsd"],  
                                output_dim=train_data.images.shape[-3:],
                                num_basis=configs[key]["Num_Bases"],
                                never_invalid=False, cell_type = configs[key]["cell"],
        ...
        Training_Loss = gin.training( gin, train_data.images, train_valid, train_data.target, valid_data.images, valid_valid, valid_data.target,
                                    test_data.images, test_valid, test_data.target, epochs, batch_size,
                                    x_epoch, record, fig, ax0, draw_fig= bool(configs[key]["draw_fig"]))
        Test_Loss = gin.testing( gin, test_data.images, test_valid, test_data.target, batch_size)
